//! 暗棋 NNUE 量化推理网络模块
//!
//! - `feature`:  累加器与增量差分（红黑双视角）
//! - `network`:  量化网络前向（`NnueEvaluator`）与增量评估包装（`NnueBoard`）
//! - `adapter`:  与 `banqi-core` `NnueEvaluate`/`NnueAccumulator` trait 的桥接

pub mod adapter;
pub mod feature;
pub mod network;

pub use adapter::NnueEngineExt;
